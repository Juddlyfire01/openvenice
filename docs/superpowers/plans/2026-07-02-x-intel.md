...
**Goal:** Add a privacy-first, account-agnostic "Intel" tab to OVX that gathers X posts/profiles via the user's own bearer token, persists per-target intel reports in the browser, and produces Venice-synthesized character profiles + engagement network graphs.

**Architecture:** A new `x-intel` module (lib + stores + components) bolted onto the existing OVX SPA. Reads hit the X API v2 through a Vite proxy (`/xapi` → `https://api.x.com`) mirroring the existing `/venice` proxy. All state persists in the browser via Zustand persist (localStorage/IndexedDB); Venice synthesis reuses `venice-client.ts`. No backend, no posting in v1.
